...
def getProd(id):
    ...
    ps_arr = []
    for p in ps:
        ps_arr.append({'id':p.id,'name':p.name,'expiry':p.expiry,'rate':p.rate,'quantity':p.quantity,'units':p.units,'imgid':p.images[0].id,'added':p.added})
    return ps_arr,200
...
def addProd():
    ...
    if name is not None and expiry is not None and rate is not None and quantity is not None and cid is not None:
        ...
        models.Image.create(image.read(),p)
        return 'OK',200
    else:
        return 'FAILED',478
...
def editProd():
    ...
    if id is not None and name is not None and expiry is not None and rate is not None and quantity is not None and cid is not None:
        models.Product.edit(id,name,expiry,rate,quantity,cid,units,image)
    return 'OK',200
...
@cache.memoize(1000)
def getimg(id):
    img = models.Image.query.filter_by(id=id).one_or_none()
    return Response(img.data, content_type='image/jpeg')
