...
class Product(db.Model):
    ...
    @staticmethod
    def edit(id,name,expiry,rate,quantity,cid,units,image):
        p = Product.query.filter_by(id=id).one_or_none()
        if p:
            n = p.images[0].id
            Image.create(image.read(),p)
            p.name = name
            p.expiry = expiry
            p.rate = rate
            p.quantity = quantity
            ...
            p.units = units
            db.session.commit()
            i = Image.query.filter_by(id=n).one_or_none()
            db.session.delete(i)
            db.session.commit()
...
class Image(db.Model):
    ...
    def create(data,product):
        i = Image(data=data,product=product)
        db.session.add(i)
        db.session.commit()
